...
var Flowgen = (() => {
  ...
  // src/schema.ts
  var HexColor = external_exports.string().regex(/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/u, "expected hex color like #1c1917");
  var IconName = external_exports.enum([
    "user",
    "server",
    "user-cog",
    "shield",
  ...
  var Actor = external_exports.object({
    label: external_exports.string(),
    color: HexColor,
    icon: IconName
  });
  var Outcome = external_exports.object({
    label: external_exports.string(),
    color: HexColor
  });
  var EventLog = external_exports.object({
    color: HexColor.default("#b45309"),
    text: HexColor.default("#92400e"),
    bg: HexColor.default("#fffbeb")
  });
  var Frontmatter = external_exports.object({
    title: external_exports.string(),
    kicker: external_exports.string().optional(),
    description: external_exports.string().optional(),
    theme: external_exports.enum(["blueprint", "mono-light", "mono-dark"]).default("blueprint"),
    direction: external_exports.enum(["TB", "LR"]).default("TB"),
    max_width: external_exports.number().int().positive().default(960),
    actors: external_exports.record(external_exports.string(), Actor),
    outcomes: external_exports.record(external_exports.string(), Outcome),
  ...
  var ColorRef = external_exports.string();
  var SideBranch = external_exports.object({
    type: external_exports.literal("side_branch").optional(),
    id: external_exports.string().optional(),
    // referenceable from AuditEvent.fires_at
    variant: external_exports.enum(["async", "note"]).default("async"),
    actor: external_exports.string().optional(),
    pill: external_exports.string().optional(),
    title: external_exports.string(),
    desc: external_exports.string().optional(),
    trailing_note: external_exports.string().optional()
  });
  var Step = external_exports.object({
    ...
    // outcome key — promotes this to a terminal step
    pill: external_exports.string().optional(),
  ...
  var Fork = external_exports.object({
    type: external_exports.literal("fork"),
  ...
  var Arrow = external_exports.object({
    type: external_exports.literal("arrow"),
    label: external_exports.string().optional(),
    color: ColorRef.optional(),
    height: external_exports.number().int().positive().optional()
  });
  var Terminator = external_exports.object({
    type: external_exports.literal("terminator"),
    color: ColorRef
  });
  var Reroute = external_exports.object({
    type: external_exports.literal("reroute"),
    label: external_exports.string(),
    description: external_exports.string().optional(),
    color: ColorRef.default("outcome:blocked")
  });
  var BranchBlock = external_exports.lazy(
    () => external_exports.object({
      ...
      accent: ColorRef,
      layout: external_exports.enum(["stack", "side-by-side"]).optional(),
      children: external_exports.array(FlowItem)
    })
  );
  var ParallelBlock = external_exports.lazy(
    () => external_exports.object({
  ...
  var FlowItem = external_exports.lazy(
  ...
  var TerminalGroup = external_exports.object({
  ...
  var AuditEvent = external_exports.object({
    ...
    fires_at: external_exports.array(external_exports.string()).default([]),
    ...
    fires_at_display: external_exports.string().optional()
  });
  var Footnote = external_exports.object({
    label: external_exports.string(),
    body: external_exports.string()
  });
  var FlowDoc = external_exports.object({
    frontmatter: Frontmatter,
    flow: external_exports.array(FlowItem),
    terminal_states: external_exports.array(TerminalGroup).default([]),
    audit_events: external_exports.array(AuditEvent).default([]),
    footnotes: external_exports.array(Footnote).default([])
  });
  ...
  function splitFrontmatter(source) {
    ...
    let data;
    try {
      data = jsYaml.load(yamlBlock);
    } catch (err) {
      throw new ParseError(
        `failed to parse frontmatter YAML: ${err.message}`,
        "frontmatter",
        err
      );
    }
    if (data === null || data === void 0) return { data: {}, content };
  ...
  var ParseError = class extends Error {
    constructor(message, section, cause) {
      super(message);
  ...
  function extractSections(body) {
    ...
    for (const h of headers) {
      ...
      try {
        sections[key] = jsYaml.load(yamlSource);
      } catch (err) {
        throw new ParseError(
          `failed to parse YAML in ## ${h.name}: ${err.message}`,
          h.name,
          err
        );
      }
...
${lines.join("\n")}`,
  ...
  // src/validate.ts
  var NESTING_SOFT_CAP = 2;
  var NESTING_HARD_CAP = 3;
  function validate(doc) {
    ...
    const seenStepIds = /* @__PURE__ */ new Map();
    function resolveColor2(ref, path) {
    ...
    function walk(items, path, decisionDepth) {
      items.forEach((item, idx) => {
        ...
        switch (item.type) {
          case "step": {
            ...
            if (item.side_branch) {
              if (item.side_branch.actor && !actorKeys.has(item.side_branch.actor)) {
                issues.push({
                  severity: "error",
          ...
          case "fork": {
            item.legs.forEach(
              (leg, j) => resolveColor2(leg.color, `${here}.legs[${j}].color`)
            );
            break;
          }
          case "arrow": {
            if (item.color) resolveColor2(item.color, `${here}.color`);
            break;
          }
          case "terminator": {
            resolveColor2(item.color, `${here}.color`);
            break;
          }
          case "reroute": {
            resolveColor2(item.color, `${here}.color`);
          ...
          case "branch": {
            resolveColor2(item.accent, `${here}.accent`);
          ...
          case "parallel": {
            item.columns.forEach(
              (col, j) => walk(col.children, `${here}.columns[${j}].children`, decisionDepth)
            );
            break;
          }
        }
    ...
    function containsDecision(items) {
      return items.some(
        (i) => i.type === "decision" || i.type === "branch" && containsDecision(i.children) || i.type === "parallel" && i.columns.some((c) => containsDecision(c.children))
      );
    }
  ...
  function isRawHtml(v) {
    return !!v && typeof v === "object" && "__raw" in v;
  }
  function raw(s) {
    return {
      __raw: s,
    ...
    strings.forEach((s, i) => {
      ...
      if (i < values.length) {
        ...
        if (isRawHtml(v)) {
          ...
          out += v.map((x) => isRawHtml(x) ? x.__raw : esc(x)).join("");
        } else {
          out += esc(v);
        }
      }
  ...
  var PATHS = {
    ...
    cog: `<path d="M12 20a8 8 0 1 1 0-16 8 8 0 0 1 0 16Z"/><path d="M12 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4Z"/><path d="M12 2v2"/><path d="M12 22v-2"/><path d="m17 20.66-1-1.73"/><path d="M11 10.27 7 3.34"/><path d="m20.66 17-1.73-1"/><path d="m3.34 7 1.73 1"/><path d="M14 12h8"/><path d="M2 12h2"/><path d="m20.66 7-1.73 1"/><path d="m3.34 17 1.73-1"/><path d="m17 3.34-1 1.73"/><path d="m11 13.73-4 6.93"/>`,
  ...
  var MONO = `'JetBrains Mono', 'Fira Code', ui-monospace, SFMono-Regular, monospace`;
  var DISPLAY = `ui-serif, 'Georgia', serif`;
  function defaultArrowHeight(ctx) {
    const top = ctx.containerStack[ctx.containerStack.length - 1];
    if (top === "parallel-column") return 8;
    if (top === "branch") return 10;
    return 20;
  }
  function pushed(ctx, kind) {
    return { ...ctx, containerStack: [...ctx.containerStack, kind] };
  }
  function actorChip(actorKey, ctx) {
  ...
  >${raw(icon(a.icon, { size: 11, strokeWidth: 2.5 }))}${a.label}</span>`;
  }
  function logPill(eventName, ctx) {
  ...
  function renderStep(step, ctx) {
    const outcomeColor = step.outcome ? resolveColor(`outcome:${step.outcome}`, ctx.doc.frontmatter) : void 0;
    const stepStyle = step.outcome ? style({
    ...
    const sideBranch = step.side_branch ? renderSideBranch(step.side_branch, ctx) : raw("");
    const inner = html`<div class="px-3 py-2.5">
    <div class="flex items-center gap-2 mb-1.5">
      ${actorChip(step.actor, ctx)}
      ${step.id ? html`<span class="text-[9px] tabular-nums tracking-wider text-stone-500" style="${raw(
      style({ fontFamily: MONO })
    )}">${step.id}</span>` : raw("")}
      ${step.pill ? html`<span class="text-[9px] font-bold tracking-[0.2em] px-1.5 py-0.5 text-white" style="${raw(
      style({
        backgroundColor: ctx.doc.frontmatter.event_log.text,
        fontFamily: MONO
      })
    )}">${step.pill}</span>` : raw("")}
      ${step.outcome ? html`<span class="ml-auto text-[9px] font-bold tracking-[0.2em] px-1.5 py-0.5 text-white" style="${raw(
      style({ backgroundColor: outcomeColor, fontFamily: MONO })
    )}">${ctx.doc.frontmatter.outcomes[step.outcome].label}</span>` : raw("")}
    </div>
    <div class="text-[13px] font-bold text-stone-900 leading-tight" style="${raw(
      style({ fontFamily: MONO })
    )}">${step.title}</div>
    ${step.desc ? html`<div class="text-[11px] mt-1 leading-snug text-stone-600" style="${raw(
      style({ fontFamily: MONO })
    )}">${step.desc}</div>` : raw("")}
    ${events.length > 0 ? html`<div class="flex flex-wrap gap-1 mt-2 pt-2" style="${raw(
      style({
        borderTop: `1px dashed ${ctx.doc.frontmatter.event_log.color}`
      })
    )}">${events.map((e) => logPill(e.name, ctx))}</div>` : raw("")}
  </div>`;
    const stepHtml = html`<div class="relative bg-white" style="${raw(stepStyle)}">${inner}</div>`;
    return raw(stepHtml.__raw + sideBranch.__raw);
  }
  function renderSideBranch(sb, ctx) {
    const { color: border, text, bg } = ctx.doc.frontmatter.event_log;
    ...
    <div class="flex items-stretch">
      ...
      style({ border: `2px dashed ${border}`, backgroundColor: bg })
        ...
        <div class="px-3 py-2.5">
          <div class="flex items-center gap-2 mb-1.5">
            ...
            ${sb.pill ? html`<span class="text-[9px] font-bold tracking-[0.2em] px-1.5 py-0.5 text-white" style="${raw(
      style({ backgroundColor: text, fontFamily: MONO })
    )}">${sb.pill}</span>` : raw("")}
            ${sb.variant === "async" ? raw(
      icon("triangle-alert", {
        size: 12,
        strokeWidth: 2,
        color: text,
      ...
      style({ fontFamily: MONO })
          ...
          ${sb.desc ? html`<div class="text-[10px] mt-1 leading-snug text-stone-600" style="${raw(
      style({ fontFamily: MONO })
    )}">${sb.desc}</div>` : raw("")}
          ${events.length > 0 || sb.trailing_note ? html`<div class="flex flex-wrap gap-1 mt-2 pt-2 items-center" style="${raw(
      style({ borderTop: `1px dashed ${border}` })
    )}">
                ${events.map((e) => logPill(e.name, ctx))}
                ${sb.trailing_note ? html`<span class="text-[9px] tracking-[0.05em] px-1.5 py-0.5 text-stone-600 italic" style="${raw(
      style({ fontFamily: MONO })
    )}">${sb.trailing_note}</span>` : raw("")}
              </div>` : raw("")}
        </div>
      </div>
    </div>
  ...
  function renderArrow(arrow, ctx) {
    const color = arrow?.color ? resolveColor(arrow.color, ctx.doc.frontmatter) : "#1c1917";
    ...
    ${label ? html`<div class="text-[9px] font-bold tracking-[0.2em] px-2 py-0.5 my-1 text-white" style="${raw(
      style({ backgroundColor: color, fontFamily: MONO })
    )}">${label}</div>
        <div style="${raw(style({ width: 2, height: 6, backgroundColor: color }))}"></div>` : raw("")}
  ...
  function renderFork(f, ctx) {
    ...
    const gridCols = legs.length === 2 ? "grid-cols-2" : legs.length === 3 ? "grid-cols-3" : "grid-cols-4";
  ...
  function renderTerminator(t, ctx) {
  ...
  function renderReroute(r, ctx) {
    ...
    <div class="relative bg-white flex items-center gap-3 px-4 py-3" style="${raw(
      ...
      style({ color, fontFamily: MONO })
        ...
        ${r.description ? html`<div class="text-[11px] text-stone-600 mt-0.5" style="${raw(
      style({ fontFamily: MONO })
    )}">${r.description}</div>` : raw("")}
      </div>
  ...
  function renderBranch(b, ctx) {
    ...
    <div class="flex items-baseline gap-3 px-4 py-2 border-b" style="${raw(headerStyle)}">
      ...
      ${b.subtitle ? html`<span class="text-[10px] text-stone-500 ml-auto" style="${raw(
      style({ fontFamily: MONO })
    )}">${b.subtitle}</span>` : raw("")}
    </div>
  ...
  function renderParallel(p, ctx) {
    const colsCount = p.columns.length;
    const grid = colsCount === 2 ? "grid-cols-1 md:grid-cols-2" : colsCount === 3 ? "grid-cols-1 md:grid-cols-3" : "grid-cols-1 md:grid-cols-4";
    const innerCtx = pushed(ctx, "parallel-column");
    const cols = p.columns.map(
      (col) => html`<div>
      ${col.label ? html`<div class="text-[9px] font-bold tracking-[0.25em] text-stone-500 mb-2" style="${raw(
        style({ fontFamily: MONO })
      )}">${col.label}</div>` : raw("")}
  ...
  function renderHeader(doc) {
    ...
    <div class="flex items-end justify-between gap-4 flex-wrap">
      <div>
        ${fm.kicker ? html`<div class="text-[10px] tracking-[0.3em] text-stone-500 mb-2" style="${raw(
      style({ fontFamily: MONO })
    )}">${fm.kicker}</div>` : raw("")}
        <h1 class="text-4xl text-stone-900 font-bold tracking-tight" style="${raw(
      style({ fontFamily: DISPLAY })
    )}">${fm.title}</h1>
        ${fm.description ? html`<p class="text-[11px] text-stone-600 mt-2 max-w-3xl leading-relaxed" style="${raw(
      style({ fontFamily: MONO })
    )}">${fm.description}</p>` : raw("")}
      </div>
      ${renderLegend(doc)}
    </div>
  </div>`;
  ...
  function renderTerminalStates(doc) {
    ...
    const cards = doc.terminal_states.map((tg) => {
      ...
      <div class="text-[11px] text-stone-700" style="${raw(
        style({ fontFamily: MONO })
      )}">${tg.items.join(" \xB7 ")}</div>
    </div>`;
    });
    return html`<div class="mt-10 grid grid-cols-1 md:grid-cols-2 gap-3">${cards}</div>`;
  }
  function renderAuditIndex(doc) {
    if (doc.audit_events.length === 0) return raw("");
    const { color: border, text } = doc.frontmatter.event_log;
    const ctx = { doc, events: /* @__PURE__ */ new Map(), containerStack: ["root"] };
    const entries = doc.audit_events.map((ev) => {
      const where = ev.fires_at_display ?? (ev.fires_at ?? []).join(" \xB7 ");
  ...
  function renderFootnotes(doc) {
    if (doc.footnotes.length === 0) return raw("");
    const lines = doc.footnotes.map(
      (f) => html`<p><span class="font-bold text-stone-800">${f.label} ·</span> ${f.body}</p>`
    );
  ...
  // src/renderer.ts
  var MONO2 = `'JetBrains Mono', 'Fira Code', ui-monospace, SFMono-Regular, monospace`;
  function render(doc, opts = {}) {
    ...
    const body = `
    <div class="min-h-screen bg-stone-100 p-6" style="font-family: ${MONO2}">
  ...
  `;
    ...
    const tailwind = includeTailwindCdn ? `<script src="https://cdn.tailwindcss.com"><\/script>` : "";
    return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
kicker: self-service device recovery \xB7 DEMO
...
- type: fork
  legs:
    - { label: "YES \xB7 PASS", color: outcome:success }
    - { label: "NO \xB7 FAIL",  color: outcome:blocked }
...
`;
  function runPipeline(source) {
    try {
      ...
      const html2 = render(doc);
      return { html: html2, issues };
    } catch (err) {
      const msg = err instanceof ParseError ? err.message : err instanceof Error ? err.message : String(err);
  ...
  function renderIssues(target, statusEl, result) {
    target.innerHTML = "";
    if (result.fatal) {
  ...
  function stripHeading(code) {
    return code.replace(/^##\s+.+\r?\n+/, "");
  }
  function safePreview(snippet2) {
    try {
      return renderPreviewFor(snippet2);
    } catch (err) {
      return `<div class="cs-preview-error">preview unavailable: ${escapeHtml2(
        err.message ?? String(err)
      )}</div>`;
    }
  }
  function renderPreviewFor(snippet2) {
    const data = jsYaml.load(stripHeading(snippet2.code));
    switch (snippet2.kind) {
      case "flow": {
      ...
      case "terminals": {
        const doc = {
          frontmatter: PREVIEW_FRONTMATTER,
          flow: [],
          terminal_states: Array.isArray(data) ? data : [],
          audit_events: [],
          footnotes: []
        };
      ...
      case "events": {
        const doc = {
          ...
          flow: [],
          terminal_states: [],
          audit_events: Array.isArray(data) ? data : [],
          footnotes: []
        };
      ...
      case "footnotes": {
        const doc = {
          ...
          terminal_states: [],
          audit_events: [],
          footnotes: Array.isArray(data) ? data : []
        };
        return renderFootnotes(doc).__raw;
      }
    }
  }
  var COMPONENT_SNIPPETS = [
    {
      label: "step",
...
- type: fork
  ...
  # height: 14`,
    ...
    },
    {
      ...
      code: `- type: parallel
  label: STEPS 04 + 05 \xB7 PARALLEL
  columns:
    - label: STEP 04 \xB7 USER SEES
      children:
        - type: step
          id: "04"
          actor: user
          title: Reads primer
    - label: STEP 05 \xB7 BACKEND PREP
      children:
        - type: step
          id: "5.1"
          actor: backend
...
- label: RISK SIGNALS IN SCOPE
  body: privilege level \xB7 lifecycle status \xB7 known device`,
      kind: "footnotes"
    }
  ];
  function renderCheatsheet() {
    ...
    <div class="cs-snippet">
      <div class="label">
        <span>${escapeHtml2(s.label)}</span>
        <span class="desc">${escapeHtml2(s.desc)}</span>
        <span class="spacer"></span>
        <button class="copy" type="button" data-copy="${escapeAttr(s.code)}">Copy</button>
      </div>
      <pre>${escapeHtml2(s.code)}</pre>
      ${preview ? `<div class="cs-preview"><div class="cs-preview-label">Preview</div>${preview}</div>` : ""}
    </div>`;
    };
    const iconGrid = ACTOR_ICONS.map(
      (name) => `<div class="icon-chip">${icon(name, { size: 14, strokeWidth: 2, color: "#1c1917" })}<code>${name}</code></div>`
    ).join("");
    return `
    ...
    <section class="cs-section">
      <h2>Component types \xB7 ## Flow</h2>
  ...
  function wireCheatsheet() {
    ...
    const populate = () => {
      ...
      body.querySelectorAll("button.copy").forEach((btn) => {
